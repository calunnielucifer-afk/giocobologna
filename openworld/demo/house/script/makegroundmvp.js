...
function makeGroundMvp(){
    ...
    // 添加主角：Serena (realistic proportions and colors)
    if(true){
        // Corpo principale di Serena (parallelepipedo umano)
        const bodyIndex = k.addPhy({
            name: 'SerenaBody',
            DPZ: 0,
            colliGroup: 1,
            X: lastPos.x,
            ...
            mixValue: 0.8,
            size: mainVPSize,
            mass: 55, // leggermente più leggera di un uomo medio
            background: '#FFB6C1', // rosa chiaro per abito/vestito
            texture: greenStone,
        });

        // Testa (piccolo cubo sopra il corpo)
        const headIndex = k.addPhy({
            name: 'SerenaHead',
            DPZ: 0,
            colliGroup: 1,
            X: lastPos.x,
            ...
            size: mainVPSize * 0.4,
            mass: 5,
            background: '#FDBCB4', // pelle chiara
            texture: frosted,
        });

        // Capelli (cubo leggermente più largo della testa)
        const hairIndex = k.addPhy({
            name: 'SerenaHair',
            DPZ: 0,
            colliGroup: 1,
            X: lastPos.x,
            ...
            Z: lastPos.z,
            mixValue: 0.85,
            size: mainVPSize * 0.45,
            mass: 2,
            background: '#000000', // nero
            texture: frosted,
        });

        // Imposta il corpo principale come player per il motore
        k.mainVPlayer = bodyIndex;

        // Collega testa e capelli al corpo (se il motore supporta vincoli)
        if (k && k.addConstraint) {
            k.addConstraint({ type: 'fixed', bodyA: bodyIndex, bodyB: headIndex });
            k.addConstraint({ type: 'fixed', bodyA: headIndex, bodyB: hairIndex });
        }

        if (k?.centerDot) {
            k.centerDot.setCamView(2); // mantiene la visuale di default
        }
    }

    // 配置默认的运动速度 (più realistici per Serena)
    if(true){
        k.SPRINT_MIN_SPEED = 3; // camminata lenta
        k.SPRINT_MAX_SPEED = 8; // corsa moderata
    }
